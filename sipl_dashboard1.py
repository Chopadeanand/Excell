  ...
  return {{
    ...
    options: {{
      ...
      scales: {{
        x: {{
          grid: {{ display: false }},
          ticks: {{ font: {{ size: isDiv ? 10 : 11 }}, maxRotation: isDiv ? 35 : 0 }}
        }},
        y: {{
...
function show(key) {{
  ...
  const title = 'HiRATE Observations \u2014 ' + key + ' ' + curMonth;
  ...
  document.getElementById('kpi-obs-n').textContent = d.obs.toLocaleString() + ' observations';
